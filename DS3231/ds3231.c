 ...
 * - 0x11 temperature MSB (signed)
 * - 0x12 temperature LSB in two MSB bits
 */ 
...
uint8_t ds3231_set_alarm_s(uint8_t day, uint8_t hour, uint8_t min, uint8_t sec, uint8_t alarm, uint8_t mode, bool intrpt)
{
	...
	if (TWI_start_transceiver_with_data(msgBuf, 2))	// Write register address
	{
		// Handle transmission error
		return (false);
	}
	...
	if (TWI_start_transceiver_with_data(msgBuf, 2))	// Read control register
	{
		// Handle transmission error
		return (false);
	}
	...
	if (TWI_start_transceiver_with_data(msgBuf, 3))	// Write to control register
	{
		// Handle transmission error
		return (false);
	}
